  ...
  const postOverlayResistanceDuration = 300; // ms to keep extra resistance after overlays
    ...
    contentEl.addEventListener('wheel', (ev) => {
      ...
      if (Math.abs(ev.deltaY) > Math.abs(ev.deltaX)) {
        ...
        ev.preventDefault();
        // Use the smoothed scroll mapping for all wheel input to keep a
        // consistent, guided pace across the entire page (no special final
        // passthroughs). This preserves the reveal behavior while making the
        // experience predictable for users.
    ...
    contentEl.addEventListener('touchmove', (ev) => {
      ...
      // Always use the same smoothed mapping for touch input so scrolling
      // pace remains consistent across the entire page.
